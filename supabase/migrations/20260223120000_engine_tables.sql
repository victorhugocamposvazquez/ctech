-- ============================================================
-- CTech Engine: tablas para wallet intelligence, token health,
-- market regime, risk state y campos nuevos en trades.
-- ============================================================
...
create policy "tracked_wallets_update_own" on public.tracked_wallets for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "token_registry_select_own" on public.token_registry for select using (auth.uid() = user_id);
create policy "token_registry_insert_own" on public.token_registry for insert with check (auth.uid() = user_id);
create policy "token_registry_update_own" on public.token_registry for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "token_registry_delete_own" on public.token_registry for delete using (auth.uid() = user_id);
...
create policy "market_regimes_select_own" on public.market_regimes for select using (auth.uid() = user_id);
create policy "market_regimes_insert_own" on public.market_regimes for insert with check (auth.uid() = user_id);
...
create policy "risk_state_select_own" on public.risk_state for select using (auth.uid() = user_id);
create policy "risk_state_insert_own" on public.risk_state for insert with check (auth.uid() = user_id);
create policy "risk_state_update_own" on public.risk_state for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- ---------- Nuevos campos en trades ----------
alter table public.trades
  ...
  add column if not exists slippage_simulated numeric(10,6),
  add column if not exists gas_simulated numeric(18,8),
  add column if not exists latency_ms int,
  add column if not exists entry_reason text,
  add column if not exists exit_reason text,
  add column if not exists wallet_movement_id uuid references public.wallet_movements(id) on delete set null,
  add column if not exists token_health_score_at_entry numeric(6,2),
  add column if not exists wallet_score_at_entry numeric(6,2);
