...
/**
 * @title FlashUSDTLab
 * @notice TRC-20 lab token — simula estafas Flash USDT (modo token falso + modo flash pendiente).
 * @dev Modo pending_flash: balanceOf inflado temporalmente + evento Transfer; no transferible.
 */
contract FlashUSDTLab {
    ...
    /**
     * @dev Incluye crédito flash activo — replica saldo temporal que ve la wallet.
     */
    ...
    /**
     * Modo 1 — transfer() real treasury→alumno (token falso permanente hasta burn).
     */
    ...
    /**
     * Modo 2 — Flash USDT pendiente: emite Transfer + infla balanceOf temporalmente.
     * El crédito flash NO es transferible (solo _balances reales pueden moverse).
     * Al expirar, balanceOf cae sin tx adicional — como en estafas "flash".
     */
    function flashInject(address to, uint256 amount, uint256 durationSeconds) external onlyOwner {
        ...
        require(durationSeconds >= 60 && durationSeconds <= 7 days, "FlashUSDTLab: bad duration");
